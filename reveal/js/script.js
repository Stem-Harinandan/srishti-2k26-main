...
document.addEventListener("click", (e) => {
  ...
  if (!targetEl) return;
  e.preventDefault();
  if (window.srishtiLenis) {
    window.srishtiLenis.scrollTo(targetEl, { offset: -80 });
  } else {
...
/* =========================================================
   EVENTS — MOBILE SNAP + DOTS + KEYBOARD CONTROLS
========================================================= */
...
function updateEventsDots() {
  ...
  const activeIndex = Math.round(
    eventsGrid.scrollLeft / eventCards[0]?.offsetWidth || eventsGrid.clientWidth,
  );
  dots.forEach((dot, i) => dot.classList.toggle("active", i === activeIndex));
}

// Keyboard navigation for events on mobile
if (eventsGrid) {
  document.addEventListener("keydown", (e) => {
    if (window.innerWidth > 768) return;
    if (e.key === "ArrowRight") {
      const activeIndex = Array.from(eventsDots?.querySelectorAll(".dot.active") || []).length > 0 
        ? Array.from(eventsDots.querySelectorAll(".dot")).findIndex(d => d.classList.contains("active"))
        : -1;
      const nextIndex = Math.min(activeIndex + 1, eventCards.length - 1);
      if (nextIndex >= 0) eventCards[nextIndex].scrollIntoView({ behavior: "smooth", inline: "start" });
    } else if (e.key === "ArrowLeft") {
      const activeIndex = Array.from(eventsDots?.querySelectorAll(".dot.active") || []).length > 0
        ? Array.from(eventsDots.querySelectorAll(".dot")).findIndex(d => d.classList.contains("active"))
        : 0;
      const prevIndex = Math.max(activeIndex - 1, 0);
      eventCards[prevIndex].scrollIntoView({ behavior: "smooth", inline: "start" });
    }
  });
...
/* =========================================================
   HOVER SOUND — throttled to prevent overlapping
========================================================= */
const revealSound = document.getElementById("revealSound");
let lastRevealSoundTime = 0;
const REVEAL_SOUND_THROTTLE = 100; // ms

function playReveal() {
  if (!revealSound) return;
  const now = performance.now();
  if (now - lastRevealSoundTime < REVEAL_SOUND_THROTTLE) return;
  
  lastRevealSoundTime = now;
...
/* =========================================================
   ...
   Combines navbar, progress, reveals, and parallax
========================================================= */
